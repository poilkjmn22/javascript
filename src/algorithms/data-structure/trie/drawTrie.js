import IntTrie from "./IntTrie/IntTrie";
import IntTree from "./IntTree/IntTree";
import Trie from "./Trie/Trie";
import Patricia from "./Patricia/Patricia";
import SuffixTree from "./SuffixTree/SuffixTree";
import { angleToRad } from "@/geometry/utils";
import { makeArrowDef } from "@/utils/svg";
...
export function drawSuffixTreeByD3(elContainer) {
  const { offsetWidth, offsetHeight } = elContainer;
  const t5 = SuffixTree.build("mississippi");
  const svg = d3
    .create("svg")
    .attr("class", "svg-container")
    .attr("viewBox", [0, 0, offsetWidth, offsetHeight]);
  ...
  const mainColor = "#1A6EEA";
  const drawLine = (n1, n2, g, c, strRef) => {
    const line = d3.line()([
      [n1.x, n1.y],
      [n2.x, n2.y],
    ]);
    ...
    g.append("text")
      .attr("style", `font-size: 18px;stroke: #333;`)
      .attr("text-anchor", "middle")
      .text(t5.str.substring(strRef[0], strRef[1] + 1))
      .attr("x", (n1.x + n2.x) / 2)
      .attr("y", (n1.y + n2.y) / 2)
      .attr("stroke", "white");
  };
  const drawNode = (n, p, branch, c, strRef) => {
    if (!p) {
      n.x = x0;
      n.y = y0;
      n.level = 0;
      ...
      const childrenLength = p.children.size;
      const mid = (childrenLength - 1) >> 1;
      n.x = p.x + siblingGap * Math.pow(n.level, 1.3) * (branch - mid);
      n.y = p.y + levelHeight;
      drawLine(p, n, g, c, strRef);
    }
    const formatNodeDisplay = (n) => n.value || "";
    const gn = g
      .append("g")
  ...
  const drawSuffixLink = (n) => {
    // 绘制后缀链接
    const suffix_link = d3
      .line()
    ...
    if (n.suffix) {
      g.append("path")
        .attr("class", "line-suffix-link")
        .attr("d", suffix_link([n, n.suffix]))
        .attr("stroke", "red")
        .attr("stroke-width", 2)
  ...
  t5.traverse(drawNode);
  makeArrowDef(svg);
  t5.traverse(drawSuffixLink);
