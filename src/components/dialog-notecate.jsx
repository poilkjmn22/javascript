...
class DialogNotecate extends React.Component {
  constructor(props) {
    ...
    this.handleChangeUpdateNotecate = this.handleChangeUpdateNotecate.bind(this)
  ...
  render() {
    ...
    return (
      <div className={`dialog-box notecate ${isShow ? '' : 'display-none'}`}>
        <h2 className="dialog-header">{ notecate.name } <span className="btn-close with-icon" onClick={this.handleClose}><i className="iconfont icon-close"></i></span></h2>
        <div className="dialog-body">
          <div className="handle-type">
            <button className="button-base button-primary" onClick={this.handleClickAddNotecate}>新增子分类</button>
            <input className={this.state.handleType === 'ADD' ? '' : 'display-none'} ref="inputNotecate" value={this.state.addNotecateTitle} onChange={this.handleChangeAddNotecate} type="text" onKeyUp={this.handleKeyupAddNotecate} />
          </div>
          <div className="handle-type">
            <button className="button-base button-primary" onClick={this.handleClickUpdateNotecate}>编辑</button>
            <input className={this.state.handleType === 'UPDATE' ? '' : 'display-none'} ref="updateNotecate" value={this.state.updateNotecate} onChange={this.handleChangeUpdateNotecate} type="text" onKeyUp={this.handleKeyupUpdateNotecate} />
          </div>
          <div className="handle-type">
            <button className="button-base button-delete" onClick={this.handleDeleteNotecate}>删除</button><br />
          </div>
        </div>
      </div>
    )
  ...
  async handleKeyupAddNotecate(event){
    if(event.keyCode === 13){
      ...
      const res = await axios.post('/api/note-cate/add', {
  ...
  async handleDeleteNotecate() {
    ...
    if (confirm) {
      const res = await axios.post('api/note-cate/delete', this.props.notecate)
        .catch(console.error)
      if (res.data.code === 0) {
  ...
  async handleKeyupUpdateNotecate() {
    if(event.keyCode === 13){
      if(!event.target.value) return
      const res = await axios.post('api/note-cate/update', { 
