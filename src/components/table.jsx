...
class Table extends React.Component{
  ...
  render(){
    ...
    for(let k of tableProps){
      editingTds.push(
        <td key={k}>
          <input name={k} value={this.state.editingRow[k]} onChange={this.handleChangeEditingTd}  />
        </td>
      )
    }
    return (
      <table className={this.props.name}>
        ...
        <tbody>
          ...
          <tr className={['editing-row', this.state.isEditing ? '' : 'display-none'].join(' ')}>
            {editingTds}
          </tr>
        </tbody>
        <tfoot>
          <tr className="opra-row">
            <td colSpan="2"><button onClick={this.handleSave} className="button-base">OK</button><button onClick={this.handleClickEditing} className="button-base">+</button><button className="button-base button-delete" onClick={this.handleDelete}>-</button></td>
  ...
  handleSave(){
    api('post', 'nav-items/add', Object.assign({}, this.state.editingRow, {
