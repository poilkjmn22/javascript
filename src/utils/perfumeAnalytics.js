...
import { debounce, sortBy, pick } from 'lodash-es';
...
class PerfumeAnalytics {
  ...
  track(metricName, data) {
    if (metricName !== 'resourceTiming') {
      ...
      const trackInfo = Object.assign(pick(obj, [
        'name',
        // 'transferSize',
        'encodedBodySize',
        // 'decodedBodySize',
        ...
        routeName: this.buildRouteMetricName(router.currentRoute.value, '')
      });
      const isExist = this.resourceTiming.find(r => r.name === trackInfo.name)
      if (isExist) {
        return;
      }
